...
class VM:
    ...
    def internal_action(self):
        ...
        message = json.dumps({"sender": self.vm_id, "clock": self.clock.value}).encode("utf-8")
    ...
    def setup_peer_connection(self, destination_vm_id: int):
        """Establish a persistent outgoing connection to a single peer"""
        if str(destination_vm_id) in self.peers:
            ...
            try:
                ...
                self.logger.info(f"Connected persistently to peer VM {destination_vm_id} on port {peer_port}")
            except Exception as e:
                self.logger.error(f"Error connecting to peer VM {destination_vm_id} on port {peer_port}: {e}")

    def recv_message(self):
        # TODO: socket logic to use message queue instead
        message = self.socket.recv(1024)
        msg_decoded = json.loads(message.decode("utf-8"))
        self.logger.info(f"Received message from VM {self.vm_id}")  # update logical clock
    ...
    def send_message(self, message: bytearray, destination_vm_id: int):
        ...
        if sock:
            try:
                ...
                self.logger.error(f"Error sending message to VM {destination_vm_id}: {e}")
        else:
    ...
    def accept_connections(self):
        # continuously accept incoming connections from other vms being added
        while True:
